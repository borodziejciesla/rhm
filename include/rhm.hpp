...
class RhmTracker {
 private:
  ...
  static constexpr double lambda_ =
      std::pow(alpha_, 2u) * (static_cast<double>(n_) + kappa_) - static_cast<double>(n_);
 ...
 public:
  explicit RhmTracker(const RhmCalibration& calibrations) : calibrations_{calibrations} {
    state_ = calibrations_.initial_state;

    kinematic_process_noise_ = ConvertDiagonalToMatrix<kinematic_state_size>(
        calibrations_.process_noise_kinematic_diagonal);
    extent_process_noise_ =
        ConvertDiagonalToMatrix<extent_state_size>(calibrations_.process_noise_extent_diagonal);
  ...
  const ObjectStateRhm& GetEstimatedState(void) const { return state_; }
 ...
 protected:
  ...
  void FirstEstimation(const std::vector<Measurement>& measurements) {
    ...
    const auto [x_min, x_max] = std::minmax_element(
        ...
        [](const Measurement& a, const Measurement& b) { return a.value(0u) < b.value(0u); });
    const auto [y_min, y_max] = std::minmax_element(
        measurements.begin(), measurements.end(),
        [](const Measurement& a, const Measurement& b) { return a.value(1u) < b.value(1u); });
    ...
    const auto [min_x, max_x] =
        std::minmax_element(points_rotated.begin(), points_rotated.end(),
    ...
    state_.extent.state(0u) =
        std::max(0.5 * (max_x->first - min_x->first), 0.5 * (max_y->second - min_y->second));
  }
  ...
  Eigen::Vector<double, extent_state_size> crosscovariance_extent_pseudomeasurement_ =
      Eigen::Vector<double, extent_state_size>::Zero();
  Eigen::Vector<double, kinematic_state_size> crosscovariance_kinematic_pseudomeasurement_ =
      Eigen::Vector<double, kinematic_state_size>::Zero();
  Eigen::Matrix<double, kinematic_state_size, measurement_size>
      crosscovariance_kinematic_measurement_ =
          Eigen::Matrix<double, kinematic_state_size, measurement_size>::Zero();
  Eigen::Matrix<double, measurement_size, measurement_size> covariance_measurement_ =
 ...
 private:
  ...
  void RunCorrectionStep(const std::vector<Measurement>& measurements) {
    for (const auto& measurement : measurements) MakeOneDetectionCorrection(measurement);
  }
  ...
  void MakeKinematicCorrection(const Measurement& measurement) {
    ...
    // Make correction
    state_.kinematic.state += crosscovariance_kinematic_measurement_ *
                              inversed_measurement_covariance *
                              (measurement.value - h_ * state_.kinematic.state);
    state_.kinematic.covariance -= crosscovariance_kinematic_measurement_ *
                                   inversed_measurement_covariance *
                                   crosscovariance_kinematic_measurement_.transpose();

    // Force symetricity
    state_.kinematic.covariance =
        MakeMatrixSymetric<kinematic_state_size>(state_.kinematic.covariance);
  }
  ...
  double EvaluateRadialFunction(
      const Eigen::Vector<double, 2u * extent_state_order + 1u>& fourier_coefficient,
      const Eigen::Vector<double, 2u * extent_state_order + 1u>& shape_parameters) const {
    const auto radius_matrix = fourier_coefficient.transpose() * shape_parameters;
    return radius_matrix(0u);
  }

  double CalculatePseudoMeasurementSquare(
      const Measurement& measurement,
      const Eigen::Vector<double, extent_state_size + 3u>& shape_parameters) {
    /* Find phi */
    const auto center = h_ * state_.kinematic.state;
    const auto phi =
        std::atan2(measurement.value(1u) - center(1u), measurement.value(0u) - center(0u));
    ...
    Eigen::Vector<double, 2u * extent_state_order + 1u> shape =
        shape_parameters.head(2u * extent_state_order + 1u);

    const auto radius =
        EvaluateRadialFunction(fourier_coeffs, shape.head(2u * extent_state_order + 1u));

    /* Pseudomeasurement */
    const auto diff = std::pow((center - measurement.value).norm(), 2u);
    const auto tmp = (std::pow(s, 2u) * std::pow(radius, 2u) +
                      2.0 * s * radius * e.transpose() * v + std::pow(v.norm(), 2u));
  ...
  void CalculateMeasurementCovariance(const Measurement& measurement) {
    ...
    covariance_measurement_ = h_ * state_.kinematic.covariance * h_.transpose() +
                              u * (1.0 / 12.0) * u.transpose() + measurement.covariance;
  }
  ...
  double CalculatePseudomeasurementCovariance(const double z) {
    ...
    return std::accumulate(sigma_points_predicted_weighted_cov.begin(),
                           sigma_points_predicted_weighted_cov.end(), 0.0);
  }

  Eigen::Vector<double, extent_state_size> CalculateShapePseudomeasurementCroscovariance(
      const double z) {
    Eigen::Vector<double, extent_state_size> cov_py =
        Eigen::Vector<double, extent_state_size>::Zero();
    for (auto index = 0u; index < sigma_points_number_; index++)
      cov_py += wc_.at(index) * (sigma_points_predicted_.at(index) - z) *
                (sigma_points_.at(index).head(extent_state_size) - state_.extent.state);
    return cov_py;
  }
  ...
  void SetUkfWeights(void) {
    ...
    wc_.at(0u) =
        (lambda_ / (static_cast<double>(n_) + lambda_)) + (1.0 - std::pow(alpha_, 2u) + beta_);
    std::fill(wc_.begin() + 1u, wc_.end(), 1.0 / (2.0 * (static_cast<double>(n_) + lambda_)));
  }

  void CalculateSigmaPoint(void) {
    const Eigen::Matrix<double, extent_state_size + 3u, extent_state_size + 3u> chol =
        c_ukf_.llt().matrixL();
  ...
  void PredictSigmaPointsPseudomeasurements(const Measurement& measurement) {
    std::transform(
        sigma_points_.begin(), sigma_points_.end(), sigma_points_predicted_.begin(),
  ...
  double CalculatePredictedPseudomeasurement(void) {
    ...
    return std::accumulate(sigma_points_predicted_weighted.begin(),
                           sigma_points_predicted_weighted.end(), 0.0);
  }
  ...
  Eigen::Vector2d CalculateExtentPartOfMeasurementModel(const double phi) {
    ...
    const auto fourier_coeffs = CalculateFourierCoeffs(phi);
    const auto radius = EvaluateRadialFunction(
        fourier_coeffs, state_.extent.state.head(2u * extent_state_order + 1u));
  ...
  Eigen::Matrix<double, 2u, kinematic_state_size> h_ =
      Eigen::Matrix<double, 2u, kinematic_state_size>::Zero();

  Eigen::Matrix<double, kinematic_state_size, kinematic_state_size> kinematic_process_noise_ =
      Eigen::Matrix<double, kinematic_state_size, kinematic_state_size>::Zero();
  Eigen::Matrix<double, extent_state_size, extent_state_size> extent_process_noise_ =
      Eigen::Matrix<double, extent_state_size, extent_state_size>::Zero();

  double phi_ = 0.0;

  Eigen::Matrix<double, extent_state_size + 3u, extent_state_size + 3u> c_ukf_ =
      Eigen::Matrix<double, extent_state_size + 3u, extent_state_size + 3u>::Zero();
  Eigen::Vector<double, extent_state_size + 3u> p_ukf_ =
      Eigen::Vector<double, extent_state_size + 3u>::Zero();
