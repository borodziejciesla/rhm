...
Eigen::Matrix<double, matrix_size, matrix_size> MakeMatrixSymetric(
    Eigen::Matrix<double, matrix_size, matrix_size>& matrix) {
...
const Eigen::Matrix<double, a_rows * b_rows, a_cols * b_cols>& KroneckerProduct(
    const Eigen::Matrix<double, a_rows, a_cols>& a,
    const Eigen::Matrix<double, b_rows, b_cols>& b) {
    ...
    for (size_t col = 0u; col < a_cols; col++) {
      ...
      for (auto updated_row = 0; updated_row < b_rows; updated_row++) {
        for (auto updated_col = 0; updated_col < b_cols; updated_col++) {
          output(updated_row + output_row_index, updated_col + output_col_index) =
              new_value(updated_row, updated_col);
        }
      }
    }
  }
...
Eigen::Matrix<double, size, size> ConvertDiagonalToMatrix(
    const std::array<double, size>& diagonal) {
  static Eigen::Matrix<double, size, size> output = Eigen::Matrix<double, size, size>::Zero();

  for (size_t index = 0u; index < size; index++) output(index, index) = diagonal.at(index);
